...
def loss(N, A, B):
    ...
    # Ensure B is a column vector (n, 1)
    if B.ndim == 1:
        B = B.reshape(-1, 1)

    if N.ndim == 1:
        N = N.reshape(-1, 1)
    elif N.ndim == 2 and (N.shape[0] == 1 or N.shape[1] == 1):
        N = N.reshape(-1, 1)

    # Truncate (or align) N to match B length
    if N.shape[0] != B.shape[0]:
        N = N[: B.shape[0], :]

    m = N.shape[1]
    residual = A.dot(N) - B
    loss = np.sum(np.square(residual)) / m
...
def multigrade_dnn_model_grade_1(trained_variable, data, layers_dims, opt_parameter, stop_criterion, max_learning_rate, min_learning_rate, epochs, activation):
    ...
    for i in range(epochs+1):
        ...
        if i%opt_parameter["REC_FRQ"]==0:
            ...
            #----------------cost
            train_losss.append(loss(train_predict, data['AA'], data['BB']))
            #---------------rse
            ...
            if i > 0:
                rel_error = (train_losss[-2] - train_losss[-1]) / train_losss[-1]
    ...
    trained_variable["train_losss"].append(train_losss)
...
def multigrade_dnn_model_grade_ell(trained_variable, data, prev_info, layers_dims, opt_parameter, stop_criterion,  max_learning_rate, min_learning_rate, epochs, activation):
    ...
    trained_variable["multrain_Y"]  = multrain_Y - train_N   
    trained_variable["train_losss"].append(train_losss)
    trained_variable["mul_parameters"].append(parameters)
